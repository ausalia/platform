...
import { DAYS, buildSensorLayout, eventsForDay, rng, simNode } from "./sim";
...
describe("seeded simulation", () => {
  ...
  it("differs between plants", () => {
    expect(simNode("plant-1", "good").ae).not.toEqual(simNode("plant-2", "good").ae);
  });
  it("keeps values in sane ranges", () => {
    const n = simNode("x", "critical");
    expect(n.ae).toHaveLength(DAYS);
...
describe("eventsForDay", () => {
  it("caps events at 5 per day", () => {
    expect(eventsForDay([50], 0, 2)).toHaveLength(5);
    expect(eventsForDay([0], 0, 2)).toHaveLength(0);
  });
});
